...
public class AStar {
	...
	public double[][] GraphArray(LinkedList<Point> points, LinkedList<LineNode> lines){
		double graph[][] = new double[points.size()][points.size()];
		
		for(LineNode line: lines) {
			int a = line.pt1.indexInSet(points);
			int b = line.pt2.indexInSet(points);
			if(a == -1 || b == -1) {
				line.pt1.printPoint();
				line.pt2.printPoint();
			}
	...
	public AStar(Graph myGraph, LinkedList<Point> points, LinkedList<LineNode> lines, Point start, Point end){
		...
		while(i > 1) {
			for(int j = 0; j < i - 1; j++) {
				if(CollisionDetection.graphLine(myGraph, path.get(i), path.get(j))) {
					if(j != i - 2) continue;
					else {
						i = j;
						break;
